...
async function 读取静态地址列表(env, txt = 'LINK.txt') {
	...
	try {
		const response = await env.ASSETS.fetch(`https://assets.local/${txt}`);
		if (!response.ok) return '';
		return (await response.text()).trim();
	} catch (error) {
		console.log(`读取静态${txt}失败:`, error);
		return '';
	}
}
...
export default {
	async fetch(request, env) {
		...
		if (!([mytoken, fakeToken, 访客订阅].includes(token) || url.pathname == ("/" + mytoken) || url.pathname.includes("/" + mytoken + "?"))) {
			...
			const 静态地址列表 = await 读取静态地址列表(env, 'LINK.txt');
			if (env.KV) {
				await 迁移地址列表(env, 'LINK.txt');
				if (userAgent.includes('mozilla') && !url.search && !静态地址列表) {
